...
# filter the subjects who have a first incongruent trial 
ids_A <- d_main_seq_nr %>% group_by(submission_id)%>% filter(sequence =="A") %>% select(submission_id) %>% as.list()
d_main_seq_update <- d_main_seq_nr %>% select(d_main_seq_nr$submission_id[sequence =="A"])
# look at the trials which start with A 
d_main_seq_nr %>% group_by(submission_id) %>% which(submission_id %in% ids_A) %>%
